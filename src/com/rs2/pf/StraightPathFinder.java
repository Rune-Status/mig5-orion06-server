...
public class StraightPathFinder implements PathFinder {
    ...
    public static Position findPath(Position start, Position end, boolean clipped) {
        Path p = new Path();
        ...
        for (int i = 0; i <= longest; i++) {
            numerator += shortest;
            if (!last.equals(current) && clipped && !Misc.checkClip(current, last, true)){
            	int x = p.getPoints().getLast().getX();
                int y = p.getPoints().getLast().getY();
                return new Position(x, y, start.getZ());
            }
            last = current.clone();
            p.getPoints().add(new Point(current.getX(), current.getY()));
