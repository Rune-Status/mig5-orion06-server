...
public class ObjectHandler {
	...
	public void tick() {
		ArrayList<GameObject> toUpdate = new ArrayList<GameObject>();
		...
		for (GameObject o : objects) {
			if (o.tick > 0 && o.tick < 99999) {
				o.tick--;
					...
					toRemove.add(o);
					updateObject(o);
					if((o.getDef().getId() <= 2078 && o.getDef().getId() > 2073) || //banana trees
						(o.getDef().getId() <= 1413 && o.getDef().getId() > 1408) || //pineapple plant
						(o.getDef().getId() >= 10725 && o.getDef().getId() <= 10727) //creature graveyard
						){
						toUpdate.add(o);
					}
			}
		...
		for (GameObject o : toUpdate) {//banana trees
			if (CreatureGraveyard.handleBonesRestore(o)) {
				
			} else {
				int x = o.getDef().getPosition().getX();
				int y = o.getDef().getPosition().getY();
				int z = o.getDef().getPosition().getZ();
				int face = o.getDef().getFace();
				int type = o.getDef().getType();
				new GameObject(o.getDef().getId()-1, x, y, z, face, type, o.getDef().getId()-1, 10);
			}
		}
		toRemove.clear();
		toUpdate.clear();
	...
	public void loadObjects(Player c) {
		...
		for (GameObject po : c.playerObjects) {
			if(objects.size() == 0){
				if (loadForPlayer(po, c)) {
					c.playerObjectsToRemove.add(po);
					c.getActionSender().sendObject(po.newId, po.getDef().getPosition().getX(), po.getDef().getPosition().getY(), po.getDef().getPosition().getZ(), po.faceOriginal, po.getDef().getType());
				}
			} else {
		...
		for (GameObject o : c.playerObjectsToRemove) {
			c.playerObjects.remove(o);
		}
		c.playerObjectsToRemove.clear();
		for (GameObject o : objects) {
	...
	public void addClip(int x, int y, int height, int face, int type) {
		Region.addObject(1, x, y, height, face, type, false);
		Rangable.addObject(1, x, y, height, face, type, false);
	}
    ...
    public void addObject(GameObject o, boolean clip) {
        if (getObject(o.getDef().getPosition().getX(), o.getDef().getPosition().getY(), o.getDef().getPosition().getZ()) == null) {
            objects.add(o);
            placeObject(o, clip);
        } else if (getObject(o.getDef().getPosition().getX(), o.getDef().getPosition().getY(), o.getDef().getPosition().getZ()) != null && (getObject(o.getDef().getPosition().getX(), o.getDef().getPosition().getY(), o.getDef().getPosition().getZ()).getDef().getId() < 10 || getObject(o.getDef().getPosition().getX(), o.getDef().getPosition().getY(), o.getDef().getPosition().getZ()).getDef().getId() == Constants.EMPTY_OBJECT)){
            GameObject object = getObject(o.getDef().getPosition().getX(), o.getDef().getPosition().getY(), o.getDef().getPosition().getZ());
            removeObject(object.getDef().getPosition().getX(), object.getDef().getPosition().getY(), object.getDef().getPosition().getZ(), object.getDef().getType());
            objects.add(o);
            placeObject(o, clip);
        }
    }
	...
	public static void getObjectDetails(Player player, int objectId, int x, int y) {
		...
		switch (objectId) {
			...
			case 2297 : // wildy log
				...
				return;
			case 2295 : // gnome log
				player.objectWalkX = 2474;
				player.objectWalkY = 3436;
				return;
