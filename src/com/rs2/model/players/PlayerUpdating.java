...
public final class PlayerUpdating {
	...
	public static void update(Player player) {
		...
		if (deltaX < 16 || deltaX >= 88 || deltaY < 16 || deltaY > 88) {
			...
			player.regionEnterTime = System.currentTimeMillis();
		}
		...
		if (player.getUpdateFlags().isUpdateRequired()) {
			PlayerUpdating.updateState(player, block, false, true, null);
		}
		...
		for (Iterator<Player> i = player.getPlayers().iterator(); i.hasNext();) {
			...
			if ((other.getPosition().isViewableFrom(player.getPosition()) && other.isVisible()) && other.getLoginStage() != LoginStages.LOGGED_OUT && !other.needsPlacement()) {
				...
				if (other.getUpdateFlags().isUpdateRequired()) {
					PlayerUpdating.updateState(other, block, false, false, player);
				}
			} else {
				out.writeBit(true);
				out.writeBits(2, 3);
		...
		for (int i = 0; i < World.getPlayers().length; i++) {
			...
			if (!player.getPlayers().contains(other) && (other.getPosition().isViewableFrom(player.getPosition()) && other.isVisible())) {
				...
				PlayerUpdating.updateState(other, block, true, false, player);
			}
		}
		...
		if(player.heightChange){
			ObjectHandler.getInstance().loadObjects(player);
			GroundItemManager.getManager().addGroundItems(player);
			player.heightChange = false;
		}
	}
	...
	public static void appendChat(Player player, StreamBuffer.OutBuffer out) {
		...
		int rights = player.getStaffRights();
		if(rights == 0 && player.isDonator())
			rights += 10;
	...
	public static void appendAppearance(Player player, StreamBuffer.OutBuffer out) {
		...
		if (player.transformNpc < 1) {
			...
			if(chest != null)
				hideType = chest.getDefinition().getHideType();
			if (chest == null || (chest != null && hideType != 1) && chest.validEquipment()) {
			...
			hideType = 0;
			if(helm != null)
				hideType = helm.getDefinition().getHideType();
			if (helm == null || (helm != null &&  hideType != 3 &&  hideType != 2) && helm.validEquipment()) {
			...
			// Beard.
			if (player.getGender() == Constants.GENDER_MALE && (helm == null || (helm != null &&  hideType != 4 &&  hideType != 2))) {
	...
	public static void updateLocalPlayerMovement(Player player, StreamBuffer.OutBuffer out) {
		...
		if (player.needsPlacement()) { // Do they need placement?
			...
			int posX = player.getPosition().getLocalX(player);
			int posY = player.getPosition().getLocalY(player);
	 ...
	 *            the block
	 */
	public static void updateState(Player player, StreamBuffer.OutBuffer block, boolean forceAppearance, boolean noChat, Player player1) {
		int mask = 0x0;
		...
		if (player.getUpdateFlags().isForceMovementUpdateRequired()) {
			...
			block.writeByte(player.getUpdateFlags().getStartY(), StreamBuffer.ValueType.S);*/
			if(player1 == null){
				block.writeByte(player.getUpdateFlags().getStartX(player), StreamBuffer.ValueType.S);
				block.writeByte(player.getUpdateFlags().getStartY(player), StreamBuffer.ValueType.S);
				block.writeByte(player.getUpdateFlags().getStartX(player)+player.getUpdateFlags().getEndX(), StreamBuffer.ValueType.S);
				block.writeByte(player.getUpdateFlags().getStartY(player)+player.getUpdateFlags().getEndY(), StreamBuffer.ValueType.S);
			} else {
				int xO = player.getPosition().getX()-player1.getPosition().getX();
				int yO = player.getPosition().getY()-player1.getPosition().getY();
				block.writeByte(player.getUpdateFlags().getStartX(player1)+xO, StreamBuffer.ValueType.S);
				block.writeByte(player.getUpdateFlags().getStartY(player1)+yO, StreamBuffer.ValueType.S);
				block.writeByte((player.getUpdateFlags().getStartX(player1)+xO)+player.getUpdateFlags().getEndX(), StreamBuffer.ValueType.S);
				block.writeByte((player.getUpdateFlags().getStartY(player1)+yO)+player.getUpdateFlags().getEndY(), StreamBuffer.ValueType.S);
			}
