...
public class Container {
	 ...
	 */
	public void shift() {//was commented out, possible bug somewhere??
		Item[] old = items;
	...
	public boolean add(Item item, int slot) {
		...
		if ((item.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)) && !type.equals(Type.NEVER_STACK)) {
			if (getCount(item.getId()) > 0 && item.getTimer() == -1) {
		...
		if ((item.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)) && !type.equals(Type.NEVER_STACK)) {
			for (int i = 0; i < items.length; i++) {
				if (items[i] != null && items[i].getId() == item.getId() && item.getTimer() == -1) {
					int totalCount = item.getCount() + items[i].getCount();
					if (totalCount >= Constants.MAX_ITEM_COUNT || totalCount < 1) {
						return false;
					}
					set(i, new Item(items[i].getId(), items[i].getCount() + item.getCount(), items[i].getTimer()));
					return true;
				}
			}
			if (newSlot == -1) {
			...
			if (slots >= item.getCount()) {
				...
				try {
					for (int i = 0; i < item.getCount(); i++) {
						set(slot > -1 ? newSlot : freeSlot(), new Item(item.getId(), 1, item.getTimer()));
					}
					if (b) {
						fireItemsChanged();
					}
	...
	public int remove(Item item, int preferredSlot, boolean allowZero) {
		...
		if ((item.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)) && !type.equals(Type.NEVER_STACK)) {
			...
			if (stack.getCount() > item.getCount()) {
				...
				set(slot, new Item(stack.getId(), stack.getCount() - item.getCount(), stack.getTimer()));
			} else {
				removed = stack.getCount();
				set(slot, allowZero ? new Item(stack.getId(), 0, stack.getTimer()) : null);
			}
		} else {
			for (int i = 0; i < item.getCount(); i++) {
				int slot = getSlotById(item.getId());
