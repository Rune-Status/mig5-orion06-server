...
public class Inventory {
	...
	public int addItemCount(Item item) {
		...
		itemContainer.add(new Item(item.getId(), amount, item.getTimer()));
		refresh();
		...
		if (!addItem(item)) {
			if (!item.getDefinition().isStackable()) {
				for(int i = 0; i < item.getCount(); i++)
					GroundItemManager.getManager().dropItem(new GroundItem(new Item(item.getId(), 1), player));
			} else {
	...
	public boolean addItem(Item item) {
		...
		if (!item.getDefinition().isStackable()) {
			if (amount > itemContainer.freeSlots()) {
				amount = itemContainer.freeSlots();
				int dropAmount = item.getCount()-amount;
				for(int i = 0; i < dropAmount; i++)
				GroundItemManager.getManager().dropItem(new GroundItem(new Item(item.getId(), 1, item.getTimer()), player));
			}
		}
		itemContainer.add(new Item(item.getId(), amount, item.getTimer()));
		refresh();
	...
	public boolean addItemNoGroundDrop(Item item) {
		if (item == null || !item.validItem()) {
			return false;
		}
		if (itemContainer.freeSlots() <= 0 && !itemContainer.hasRoomFor(item)) {
	...
	public void addItemToSlot(Item item, int slot) {
		...
		if (item.getDefinition().isStackable() && itemContainer.getSlotById(item.getId()) > -1) {
			...
			itemContainer.set(index, new Item(item.getId(), item.getCount() + currentItem.getCount(), item.getTimer()));
			refresh();
			return;
		}
