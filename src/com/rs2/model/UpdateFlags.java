...
import com.rs2.model.region.sound.SoundSystem;
...
public class UpdateFlags {
	...
	public void sendForceMovement(Player player, final int x, final int y, final int speed1, final int speed2, final int direction) {
		...
		this.startX = player.getPosition().getLocalX(player);
		this.startY = player.getPosition().getLocalY(player);
		this.endX = player.getPosition().getLocalX(player) + x;
		this.endY = player.getPosition().getLocalY(player) + y;
		this.moveX = x;
		this.moveY = y;
		this.speed1 = speed1;
		this.speed2 = speed2;
	...
	public void resetForceMovement() {
		startX = startY = endX = endY = speed1 = speed2 = moveX = moveY = direction = 0;
	}
	...
	public void sendGraphic(int graphicsId, int graphicsDelay) {
        if(owner.isPlayer()){
            SoundSystem.SoundData soundData = SoundSystem.SoundData.forId(graphicsId, SoundSystem.SoundType.GRAPHIC);
            if(soundData != null) ((Player)owner).getActionSender().sendSound(soundData.getSoundId(), 1, 0);
        }
		this.graphicsId = graphicsId;
	...
	public void sendAnimation(int animationId) {
        /*if(owner.isPlayer()){
            SoundSystem.SoundData soundData = SoundSystem.SoundData.forId(animationId, SoundSystem.SoundType.ANIMATION);
            if(soundData != null) ((Player)owner).getActionSender().sendSound(soundData.getSoundId(), 1, 0);
        }*/
	...
	public int getStartX(Player player) {
		return player.getPosition().getLocalX(player);
	}

	public int getStartY(Player player) {
		return player.getPosition().getLocalY(player);
	}
