...
public class JewelleryTeleport {
	...
	public static void replaceItem(Player player) {
		if(player.getStatedInterface().equals("operate")){
			if (player.getEquipment().removeItem2(new Item(player.getClickItem()))) {
				int item = findNextJewellery(player.getClickItem());
				if (item > 0) {
					player.getEquipment().replaceEquipment2(item, player.getSlot());
				}
			}
		}else{
