...
public abstract class NpcCombatDef {
	...
	public NpcCombatDef respawn(int spawnDelay_) {
		if(spawnDelay_ > 0)
	...
	public static void add(int[] ids, NpcCombatDef npcCombatDef) {
		for (int id : ids){
			...
			if(def != null && def.respawn > 0){
				double combat = def.getCombat();
				if(combat <= 0){
					npcCombatDef.spawnDelay = def.respawn;
				}else{
					double combat2 = Math.pow(combat, 2);
					double combat2_ = -0.0004*combat2;
					double combat1 = 0.3547*combat;
					double off = 24.646;
					double end = combat2_+combat1+off;
					npcCombatDef.spawnDelay = (int) end;
				}
			}
