...
public class ItemOnItemHandling {
	...
	Area darkRoom = new Area(2638, 9736, 2655, 9745, (byte) 0);//boot of lightness dark room @cammy dung.
		...
		if (itemOnItem.toAdd != null) {
			if (itemOnItem.toAdd.length == 2 && itemOnItem.toRemove.length == 2) {
				...
				for (Item items : itemOnItem.toAdd){
					...
					lit = player.isLitCandleOrLamp(items.getId());
				}
				if(lit && darkRoom.contains(player.getPosition())){
					player.getActionSender().sendMessage("The light lets you see further into the room.");
					player.teleport(new Position(player.getPosition().getX(), (player.getPosition().getY() +23), 0));
				}
			}
		}
		if(itemOnItem.emote > 0) {
