...
public class Firemaking {// todo firelighters
	...
	public void startFire(final Player player, final Firemake firemake, final int x, final int y, final int h, boolean fromGround) {
		...
		if (player.questStage[0] != 1) {
			player.getDialogue().sendStartInfo("Please wait.", "", "Your character is now attempting to light the fire.", "This should only take a few seconds.", "", true);
		}
		player.getActionSender().sendSound(375, 1, 0);
			...
			public void execute(CycleEventContainer container) {
				...
				CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
					...
					public void execute(CycleEventContainer p) {
						...
						player.getActionSender().sendSound(374, 1, 0);
						if (player.questStage[0] == 9) {
							//player.questStage[0] = 10;
                        ...
                        if (finalItem != null)
						    GroundItemManager.getManager().destroyItem(finalItem, player);
