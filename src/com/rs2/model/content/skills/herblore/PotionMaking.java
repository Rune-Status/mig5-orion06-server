...
public class PotionMaking {// todo description items
	...
	public static boolean combineDose(Player c, int itemused1, int itemused2, int slot1, int slot2) {
		...
		if (itemName1.contains("(4)") || itemName2.contains("(4)") || itemName1.contains("watering") || itemName2.contains("watering")) {
			return false;
		}
		try {
	...
	private static final double[][] POTIONS = {{249, 221, 91, 121, 25, 0}, // Attack Potion.
			...
			{2398, 6018, 5939, 5940, 175, 78}, // weapon++ Potion.
			{259, 6051, 101, 5954, 175, 78}, // antidote++ Potion.
			{2998, 6693, 3002, 6687, 175, 78}, // sara brew Potion.
	...
	public static boolean createPotion(Player player, Item useItem, Item withItem, int slot, int slotUsed) {
		...
		for (double[] data : POTIONS) {
			...
			exp = data[4];
			req = data[5];
			if (herb == 2398 || ingre == 6018 || herb == 6016 || ingre == 6049) {
			...
			if ((item == herb && usedItem == container) || (item == container && usedItem == herb)) {
				...
				createUnfinished(player, herb, unf, container, item == container ? slot : slotUsed, item == container ? slotUsed : slot);
				return true;
			}
	...
	public static void createUnfinished(final Player player, int herb, final double unf, final int vial, final int slot, final int slotUsed) {
		final Item item = new Item(herb, 1);
		final Item vial_item = new Item(vial, 1);
		if (!Constants.HERBLORE_ENABLED) {
			player.getActionSender().sendMessage("This skill is currently disabled.");
			return;
		}
		if(player.questStage[29] != 1){
			QuestDefinition questDef = QuestDefinition.forId(29);
		...
		player.getActionSender().sendMessage("You put the " + item.getDefinition().getName().replace(" herb", "").toLowerCase() + " into the "+vial_item.getDefinition().getName()+".");
	...
	public static void createFinished(final Player player, final double ingre, final double unf, final double fin, final double exp, final int slot, final int slotUsed) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if(task.getTaskTarget() == fin && player.dailyTaskAmount < task.getReqAmount()){
