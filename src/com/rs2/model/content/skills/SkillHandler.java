...
public class SkillHandler {
	...
	public static GameObjectDef getObject(int object, int x, int y, int h) {
		...
		final GameObject removed = ObjectHandler.getInstance().getObject(Constants.EMPTY_OBJECT, x, y, h);//Fixes exploits like picking up flax, when the plant is removed - not tested that much, so might cause bugs somewhere!
		if (removed != null) {
			return null;
		}
	...
	public static boolean doSpawnEvent(Player player) {
		...
		return Misc.random(2000) == 0;//was 3000
	}
