...
public class SpecialPlantTwo {
	...
	public enum SpecialPlantData {
		...
		BELLADONNA(5281, 2398, 1, 63, 320, 0.15, 91, 512, 4, 8, -1, 0, 5, 8), CACTUS(5280, 6016, 1, 55, 560, 0.15, 66.5, 25, 8, 18, 31, 374, 11, 17), BITTERCAP(5282, 6004, 1, 53, 240, 0.15, 61.5, 57.7, 4, 15, -1, 0, 12, 17);
		...
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			for (int j = 0; j < timesToGrow; j++) {
				if(farmingState[i] == 2 || farmingState[i] == 3)
					break;
				if (farmingStages[i] <= specialPlantData.getEndingState() - specialPlantData.getStartingState() + (specialPlantData == SpecialPlantData.BELLADONNA ? 3 : -2)) {
	...
	public void doStateCalculation(int index) {
		...
		if (farmingState[index] == 0 && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			if (Misc.random(100) <= maxChance) {
				...
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != specialPlantData.getStartingState()){
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateSpecialPlants();
				if (farmingStages[hopsFieldsData.getSpecialPlantsIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 0) {
			...
			if(diseaseChance[specialPlantFieldsData.getSpecialPlantsIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is one of the special patches. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[specialPlantFieldsData.getSpecialPlantsIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is one of the special patches. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if(inspectData.getMessages().length > farmingStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4)
						player.getDialogue().sendStatement(inspectData.getMessages()[farmingStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4]);
					else
						player.getDialogue().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 1]);
					container.stop();
