...
public class Allotments {
	...
	public enum AllotmentData {
		...
		POTATO(5318, 1942, 5096, 3, 1, new int[]{6032, 2}, 40, 0.30, 8, 9.5, 6, 10), ONION(5319, 1957, 5096, 3, 5, new int[]{5438, 1}, 40, 0.30, 9.5, 10.5, 13, 17), CABBAGE(5324, 1965, 5097, 3, 7, new int[]{5458, 1}, 40, 0.25, 10, 11.5, 20, 24), TOMATO(5322, 1982, 5096, 3, 12, new int[]{5478, 2}, 40, 0.25, 12.5, 14, 27, 31), SWEETCORN(5320, 5986, 6059, 3, 20, new int[]{5931, 10}, 50, 0.20, 17, 19, 34, 40), STRAWBERRY(5323, 5504, -1, 3, 31, new int[]{5386, 1}, 60, 0.20, 26, 29, 43, 49), WATERMELON(5321, 5982, 5098, 3, 47, new int[]{5970, 10}, 80, 0.20, 48.5, 54.5, 52, 60);
	...
	public int getConfigValue(int allotmentStage, int seedId, int plantState, int index) {
		...
		//System.out.println(index+" "+allotmentData.getEndingState()+" "+allotmentData.getStartingState()+" "+allotmentStage);
		//if (allotmentData.getEndingState() == allotmentData.getStartingState() + allotmentStage - 1) {
		/*if (allotmentData.getEndingState() == allotmentData.getStartingState() + allotmentStage - 4) {
			//System.out.println("fully grown: "+index);
	...
	public void doCalculations() {
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			if (farmingTimer[i] == 0 || farmingState[i] == 3 || timesToGrow <= 0) {
				continue;
			}
			for (int j = 0; j < timesToGrow; j++) {
				if(farmingState[i] == 3 || isFullyGrown(i))
					break;
	...
	boolean isFullyGrown(int index){
		...
		if(allotmentData.getStartingState() + farmingStages[index] - 4 == allotmentData.getEndingState()){
			hasFullyGrown[index] = true;
			return true;
		}
		return false;
	...
	/*void checkFullyGrown(int index){
		AllotmentData allotmentData = AllotmentData.forId(farmingSeeds[index]);
		if (allotmentData.getEndingState() == allotmentData.getStartingState() + farmingStages[index] - 4) {
			hasFullyGrown[index] = true;
			farmingState[index] = 0;
		}
	}*/
	...
	public void doStateCalculation(int index) {
		...
		/*if (farmingState[index] != 3 && farmingState[index] != 2)
			checkFullyGrown(index);*/
		if ((farmingState[index] == 0 || farmingState[index] == 1) && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			//System.out.println(index+" "+hasFullyGrown[index]);
			AllotmentData allotmentData = AllotmentData.forId(farmingSeeds[index]);
			if (allotmentData == null)
				return;
			if(farmingStages[index] > 4){
			//if(farmingStages[index] != allotmentData.getStartingState()){
				//System.out.println("allotment dis: "+farmingStages[index]+" "+allotmentData.getStartingState());
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateAllotmentsStates();
				if (farmingStages[allotmentFieldsData.getAllotmentIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			...
			if(diseaseChance[allotmentFieldsData.getAllotmentIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an allotment patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[allotmentFieldsData.getAllotmentIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an allotment patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					/*if (farmingStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length - 2) {
						System.out.println("ins1 "+farmingStages[allotmentFieldsData.getAllotmentIndex()]);
						player.getDialogue().sendStatement(inspectData.getMessages()[farmingStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
						...
						System.out.println("ins2 "+farmingStages[allotmentFieldsData.getAllotmentIndex()]);
						player.getDialogue().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 2]);
					} else {
						System.out.println("ins3 "+farmingStages[allotmentFieldsData.getAllotmentIndex()]);
	...
	public void handleFlowerProtection(int index) {
		...
		int maxChance = (int) b;
		int indexGiven = 0;
		if(player.forceDisease)
			maxChance = 100;
		if (farmingState[index] == 1) {
			//diseaseChance[index] *= (1/WATERING_CHANCE);
			farmingState[index] = 0;
			return;
		}
		...
		if (!farmingWatched[index] && Misc.random(100) <= maxChance) {
			...
			if (player.getFlowers().farmingState[indexGiven] != 3 && player.getFlowers().hasFullyGrown[indexGiven] && player.getFlowers().farmingSeeds[indexGiven] == allotmentData.getFlowerProtect()) {
				...
				if(Constants.DISEASING_ENABLED){
