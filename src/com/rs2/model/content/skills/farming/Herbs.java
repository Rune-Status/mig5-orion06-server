...
public class Herbs {
	...
	/*private static final int HARVEST_LIVES = 3;
	private static final int HARVEST_LIVES_COMPOST = 4;
	private static final int HARVEST_LIVES_SUPER_COMPOST = 5;*/
	/*private static final double COMPOST_CHANCE = 0.9;
	private static final double SUPERCOMPOST_CHANCE = 0.7;*/
	private static final double COMPOST_CHANCE = 0.35;
	private static final double SUPERCOMPOST_CHANCE = 0.1;
	...
	public enum HerbData {
		...
		GUAM(5291, 199, 9, 80, 0.25, 11, 12.5, 4, 8), MARRENTILL(5292, 201, 14, 80, 0.25, 13.5, 15, 11, 15), TARROMIN(5293, 203, 19, 80, 0.25, 16, 18, 18, 22), HARRALANDER(5294, 205, 26, 80, 0.25, 21.5, 24, 25, 29), GOUT_TUBER(6311, 3261, 29, 80, 0.25, 105, 45, 192, 196), RANARR(5295, 207, 32, 80, 0.20, 27, 30.5, 32, 36), TOADFLAX(5296, 3049, 38, 80, 0.20, 34, 38.5, 39, 43), IRIT(5297, 209, 44, 80, 0.20, 43, 48.5, 46, 50), AVANTOE(5298, 211, 50, 80, 0.20, 54.5, 61.5, 53, 57), KWUARM(5299, 213, 56, 80, 0.20, 69, 78, 68, 72), SNAPDRAGON(5300, 3051, 62, 80, 0.15, 87.5, 98.5, 75, 79), CADANTINE(5301, 215, 67, 80, 0.15, 106.5, 120, 82, 86), LANTADYME(5302, 2485, 73, 80, 0.15, 134.5, 151.5, 89, 93), DWARF(5303, 217, 79, 80, 0.15, 170.5, 192, 96, 100), TORSTOL(5304, 219, 85, 80, 0.15, 199.5, 224.5, 103, 107)
		;
	...
	public void updateHerbsStates() {
		...
		//System.out.println("Update states: "+configValues[2]);
	...
	/*public int getConfigValue(int herbStage, int seedId, int plantState, int index) {
		...
		if (herbData.getEndingState() == herbData.getStartingState() + farmingStages[index] - 4) {
			farmingState[index] = 0;
		}
		//return (getPlantState(plantState) << 6) + herbData.getStartingState() + herbStage - 4;
	...
	public int getConfigValue(int herbStage, int seedId, int plantState, int index) {
		...
		if (farmingSeeds[index] == 6311) {
			if (plantState == 1) {
				return farmingStages[index] + 193;
			} else if (plantState == 2) {
				return farmingStages[index] + 196;
			}
		}
		/*if (herbData.getEndingState() == herbData.getStartingState() + farmingStages[index] - 4) {
			farmingState[index] = 0;
		...
		//return (getPlantState(plantState) << 6) + herbData.getStartingState() + herbStage - 4;
		/*if(index == 2){
			System.out.println("getCVal: "+herbStage+" "+seedId+" "+plantState);
		}*/
		/*
		 diseased:
		 128
		 ...
		 */
		if(plantState == 1)
			return herbStage + 123;
		if(plantState == 2)
			return herbStage + 165;
		return (getPlantState(plantState) << 6) + herbData.getStartingState() + herbStage - 4;
		//return (plantState == 2 ? farmingStages[index] + 158 : plantState == 1 ? farmingStages[index] + 154 : getPlantState(plantState) << 6) + herbData.getStartingState() + herbStage - 4;
	}
	...
	public void doCalculations() {
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			if (farmingStages[i] > 0 && farmingStages[i] <= 3) {
				int times = (int) (difference / 5);
				for (int j = 0; j < times; j++) {
					if(farmingStages[i] == 0)
						break;
					farmingStages[i]--;
			...
			/*if(state > nbStates)
				state = nbStates;*/
			int timesToGrow = state - (farmingStages[i]-4);
			if (farmingTimer[i] == 0 || farmingState[i] == 2 || timesToGrow <= 0) {
				continue;
			}
			for (int j = 0; j < timesToGrow; j++) {
				if(farmingState[i] == 2 || herbData.getEndingState() == herbData.getStartingState() + farmingStages[i] - 4)
					break;
				//System.out.println("herb grow: "+herbData.getEndingState()+" "+herbData.getStartingState()+" "+farmingStages[i]+" "+farmingState[i]);
	...
	/*
	 [1.8.2014 16:32]: herb grow: 29 25 4
[1.8.2014 16:32]: herb grow: 29 25 4
[1.8.2014 16:32]: herb grow: 29 25 4
[1.8.2014 16:32]: herb grow: 29 25 4
[1.8.2014 16:32]: herb grow: 29 25 5
[1.8.2014 16:32]: herb grow: 29 25 6
[1.8.2014 16:32]: herb grow: 29 25 6
[1.8.2014 16:32]: herb grow: 29 25 6
[1.8.2014 16:32]: herb grow: 29 25 4
[1.8.2014 16:32]: herb grow: 29 25 4
	...
	boolean isFullyGrown(int index){
	...
	public void decreaseStage(int index) {
		HerbData herbData = HerbData.forId(farmingSeeds[index]);
		if (herbData == null)
			return;
		long growth = herbData.getGrowthTime();
		int nbStates = herbData.getEndingState() - herbData.getStartingState();
		int timeOfOneStage = (int) (growth / nbStates);
		farmingTimer[index] += timeOfOneStage; 
		farmingStages[index]--;
	}
	...
	public void doStateCalculation(int index) {
		...
		boolean ready = isFullyGrown(index);
		if (farmingState[index] == 0 && farmingStages[index] >= 4 && farmingStages[index] <= 7 && !ready) {
			HerbData herbData = HerbData.forId(farmingSeeds[index]);
			if (herbData == null) {
				return;
			}
			...
			if (Misc.random(100) <= maxChance) {
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != herbData.getStartingState()){
				if(Constants.DISEASING_ENABLED && farmingStages[index] > 4){
					//System.out.println("herb dis: "+farmingStages[index]+" "+herbData.getStartingState());
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateHerbsStates();
				if (farmingStages[herbFieldsData.getHerbIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		player.getActionSender().sendSound(1321, 1, 0);
	...
	public boolean harvest(int objectX, int objectY) {
		...
		if (!player.getInventory().getItemContainer().contains(FarmingConstants.SECATEURS)) {
			player.getDialogue().sendStatement("You need secateurs to harvest here.");
			return true;
		}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		final HerbData herbData = HerbData.forId(farmingSeeds[herbFieldsData.getHerbIndex()]);
		//display(herbFieldsData.getHerbIndex());
		...
		if (farmingStages[herbFieldsData.getHerbIndex()] == 0) {
			...
			if(diseaseChance[herbFieldsData.getHerbIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an herb patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[herbFieldsData.getHerbIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an herb patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
