...
public class WoodTrees {
	...
	public enum TreeData {
		...
		OAK(5370, 6043, 15, new int[]{5968, 1}, 200, 0.20, 14, 467.3, 8, 12, 13, 14, 1281), WILLOW(5371, 6045, 30, new int[]{5386, 1}, 280, 0.20, 25, 1456.3, 15, 21, 22, 23, 1308), MAPLE(5372, 6047, 45, new int[]{5396, 1}, 320, 0.25, 45, 3403.4, 24, 32, 33, 34, 1307), YEW(5373, 6049, 60, new int[]{6016, 10}, 400, 0.25, 81, 7069.9, 35, 45, 46, 47, 1309), MAGIC(5374, 6051, 75, new int[]{5976, 25}, 480, 0.25, 145.5, 13768.3, 48, 60, 61, 62, 1306);
	...
	public enum InspectData {
		OAK(5370, new String[][]{{"The acorn sapling has only just been planted."}, {"The acorn sapling grows larger."}, {"The oak tree produces a small canopy."}, {"The oak tree grows larger."}, {"The oak tree is ready to harvest."}}), WILLOW(5371, new String[][]{{"The willow sapling has only just been planted."}, {"The willow sapling grows a few small branches."}, {"The willow tree develops a small canopy."}, {"The willow tree trunk becomes dark brown,", "and the canopy grows."}, {"The willow tree trunk becomes a lighter shade", "of brown, and the canopy grows more."}, {"The trunk thickens, and the canopy grows", "yet larger."}, {"The willow tree is fully grown."}}), MAPLE(5372, new String[][]{{"The maple sapling has only just been planted."}, {"The mapling sapling grows a few small branches."}, {"The maple tree develops a small canopy."}, {"The maple tree trunk straightens and the canopy", "grows larger."}, {"The maple tree canopy grows."}, {"The maple tree canopy grows."}, {"The maple tree grows."}, {"The maple tree grows."}, {"The maple tree is ready to be harvested."}}), YEW(5373, new String[][]{
				...
				{"The magic tree grows and the base becomes lighter."}, {"The magic tree grows and the base becomes darker."}, {"The magic tree's bark is more prominent and the", "canopy gains more sparkles."}, {"The magic tree grows taller."}, {"The magic tree grows taller."}, {"The magic tree grows taller."}, {"The magic tree is ready to be harvested."}});
	...
	public void doCalculations() {
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			for (int j = 0; j < timesToGrow; j++) {
				if(farmingState[i] == 2 || isFullyGrown(i))
					break;
	...
	boolean isFullyGrown(int index){
		...
		if(treeData.getStartingState() + farmingStages[index] - 4 == treeData.getEndingState()){
			hasFullyGrown[index] = true;
			return true;
		}
		return false;
	...
	/*void checkFullyGrown(int index){
		TreeData treeData = TreeData.forId(farmingSeeds[index]);
		if (treeData.getEndingState() == treeData.getStartingState() + farmingStages[index] - 4) {
			hasFullyGrown[index] = true;
			farmingState[index] = 0;
		}
	}*/
	...
	public void doStateCalculation(int index) {
		...
		if (farmingState[index] == 0 && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			if (Misc.random(100) <= maxChance) {
				...
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != treeData.getStartingState()){
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				} else if (farmingState[hopsFieldsData.getTreeIndex()] != 7) {
					farmingStages[hopsFieldsData.getTreeIndex()] = 3;
					alreadyStopped = true;
					container.stop();
				}
				if (farmingState[hopsFieldsData.getTreeIndex()] == 7) {
					final TreeData treeData = TreeData.forId(farmingSeeds[hopsFieldsData.getTreeIndex()]);
					player.getInventory().addItemOrDrop(new Item(treeData.getRootsId()));
					farmingStages[hopsFieldsData.getTreeIndex()] = 3;
				}
				...
				updateTreeStates();
				if (farmingStages[hopsFieldsData.getTreeIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		} else if (farmingState[treeFieldsData.getTreeIndex()] == 7) {	
		...
		if (farmingStages[treeFieldsData.getTreeIndex()] == 0) {
			...
			if(diseaseChance[treeFieldsData.getTreeIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a tree patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[treeFieldsData.getTreeIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a tree patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
	...
	public boolean canCut(final int x, final int y) {
		...
		//checkFullyGrown(treeFieldsData.getTreeIndex());
