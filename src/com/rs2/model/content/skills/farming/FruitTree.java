...
public class FruitTree {
	...
	public enum FruitTreeData {
		...
		APPLE(5496, 1955, 1, 27, new int[]{5986, 9}, 960, 0.20, 22, 8.5, 8, 20, 14, 33, 34, 1199.5, 12, 18), BANANA(5497, 1963, 1, 33, new int[]{5386, 4}, 960, 0.20, 28, 10.5, 35, 47, 41, 60, 61, 1750.5, 12, 18), ORANGE(5498, 2108, 1, 39, new int[]{5406, 3}, 960, 0.20, 35.5, 13.5, 72, 84, 78, 97, 98, 2470.2, 12, 18), CURRY(5499, 5970, 1, 42, new int[]{5416, 5}, 960, 0.25, 40, 15, 99, 111, 105, 124, 125, 2906.9, 12, 18), PINEAPPLE(5500, 2114, 1, 51, new int[]{5982, 10}, 960, 0.25, 57, 21.5, 136, 148, 142, 161, 162, 4605.7, 12, 18), PAPAYA(5501, 5972, 1, 57, new int[]{2114, 10}, 960, 0.25, 72, 27, 163, 175, 169, 188, 189, 6146.4, 12, 18), PALM(5502, 5974, 1, 68, new int[]{5972, 15}, 960, 0.25, 170.5, 41.5, 200, 212, 206, 225, 226, 10150.1, 12, 18);
	...
	public void doCalculations() {
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			if (farmingTimer[i] == 0 || farmingState[i] == 2 || farmingState[i] == 3 || timesToGrow <= 0) {
				continue;
			}
			for (int j = 0; j < timesToGrow; j++) {
				if(farmingState[i] == 2 || farmingState[i] == 3)
					break;
	...
	public void doStateCalculation(int index) {
		...
		if (farmingState[index] == 0 && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			if (Misc.random(100) <= maxChance) {
				...
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != fruitTreeData.getStartingState()){
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateFruitTreeStates();
				if (farmingStages[fruitTreeFieldsData.getFruitTreeIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[fruitTreeFieldsData.getFruitTreeIndex()] == 0) {
			...
			if(diseaseChance[fruitTreeFieldsData.getFruitTreeIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a fruit tree patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[fruitTreeFieldsData.getFruitTreeIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a fruit tree patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if(inspectData.getMessages().length > farmingStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4)
						player.getDialogue().sendStatement(inspectData.getMessages()[farmingStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4]);
					else
						player.getDialogue().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 1]);
					container.stop();
