...
public class ToolLeprechaun {
	...
	public void storeItems(int itemId, int amount) {
		...
		int tempA = finalAmount;
		if(tools[toolStoreData.getToolIndex()] + tempA > toolStoreData.getToolMaxQuantity()){//new check
			finalAmount -= (tools[toolStoreData.getToolIndex()] + tempA) - toolStoreData.getToolMaxQuantity();
		}
	...
	public boolean noteItem(Npc npc, int itemId) {
		...
		for (int item : NOTABLE_ITEMS) {
			if (itemId == item && !(npc.getPosition().getX() >= 3044 && npc.getPosition().getX() <= 3064 && npc.getPosition().getY() >= 3300 && npc.getPosition().getY() <= 3313 && itemId == 1965)) {
