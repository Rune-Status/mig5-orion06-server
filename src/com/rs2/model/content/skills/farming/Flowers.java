...
public class Flowers { // todo scarecrow 6059
	...
	public enum FlowerData {
		...
		MARIGOLD(5096, 6010, 2, 20, 0.35, 8.5, 47, 8, 12), ROSEMARY(5097, 6014, 11, 20, 0.32, 12, 66.5, 13, 17), NASTURTIUM(5098, 6012, 24, 20, 0.30, 19.5, 111, 18, 22), WOAD(5099, 1793, 25, 20, 0.27, 20.5, 115.5, 23, 27), LIMPWURT(5100, 225, 26, 20, 0.215, 8.5, 120, 28, 32), ;
	...
	public int getConfigValue(int flowerStage, int seedId, int plantState, int index) {
		...
		/*if (flowerData.getEndingState() == flowerData.getStartingState() + flowerStage - 4) {
	...
	public void doCalculations() {
		for (int i = 0; i < farmingSeeds.length; i++) {
			...
			if (farmingState[i] == 3 || farmingSeeds[i] == 0x21 || farmingTimer[i] == 0 || timesToGrow <= 0) {
				continue;
			}
	...
	boolean isFullyGrown(int index){
		...
		if(flowerData.getEndingState() == flowerData.getStartingState() + farmingStages[index] - 4){
			hasFullyGrown[index] = true;
			return true;
		}
		return false;
	...
	public void doStateCalculation(int index) {
		...
		if ((farmingState[index] == 0 || farmingState[index] == 1) && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			/*if(player.getStaffRights() >= 2)
				System.out.println("flower: "+index+" "+farmingState[index]+" "+maxChance);*/
			if (Misc.random(100) <= maxChance) {
				if(Constants.DISEASING_ENABLED && farmingStages[index] > 4){
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != flowerData.getStartingState()){
					//System.out.println("flower dis: "+farmingStages[index]+" "+flowerData.getStartingState());
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateFlowerStates();
				if (farmingStages[flowerFieldsData.getFlowerIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean harvest(int objectX, int objectY) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				farmingTimer[flowerFieldsData.getFlowerIndex()] = Server.getMinutesCounter();
				player.getUpdateFlags().sendAnimation(FarmingConstants.SPADE_ANIM);
				player.getActionSender().sendMessage("You harvest the crop.");
				player.getInventory().addItemOrDrop(new Item(flowerData.getHarvestId(), flowerData.getHarvestId() == 1973 || flowerData.getHarvestId() == 225 ? 3 : 1));
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[flowerFieldsData.getFlowerIndex()] == 0) {
			...
			if(diseaseChance[flowerFieldsData.getFlowerIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an flower patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[flowerFieldsData.getFlowerIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is an flower patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
