...
public class Hops {
	...
	public enum HopsData {
		...
		BARLEY(5305, 6006, 4, 3, new int[]{6032, 3}, 40, 0.35, 8.5, 9.5, 49, 53), HAMMERSTONE(5307, 5994, 4, 4, new int[]{6010, 1}, 40, 0.35, 9, 10, 4, 8), ASGARNIAN(5308, 5996, 4, 8, new int[]{5458, 1}, 50, 0.30, 10.5, 12, 11, 16), JUTE(5306, 5931, 3, 13, new int[]{6008, 6}, 50, 0.30, 13, 14.5, 56, 61), YANILLIAN(5309, 5998, 4, 16, new int[]{5968, 1}, 60, 0.25, 14.5, 16, 19, 25), KRANDORIAN(5310, 6000, 4, 21, new int[]{5478, 1}, 70, 0.25, 17.5, 19.5, 28, 35), WILDBLOOD(5311, 6002, 4, 28, new int[]{6012, 1}, 80, 0.20, 23, 26, 38, 46);
	...
	boolean isFullyGrown(int index){
		...
		if(hopsData.getStartingState() + farmingStages[index] - 4 == hopsData.getEndingState()){
			hasFullyGrown[index] = true;
			return true;
		}
		return false;
	...
	/*void checkFullyGrown(int index){
		HopsData hopsData = HopsData.forId(farmingSeeds[index]);
		if (hopsData.getEndingState() == hopsData.getStartingState() + farmingStages[index] - 4) {
			hasFullyGrown[index] = true;
			farmingState[index] = 0;
		}else{
	...
	public void doStateCalculation(int index) {
		...
		//System.out.println("stage "+farmingStages[index]+" "+hasFullyGrown[index]);
		...
		if ((farmingState[index] == 0 || farmingState[index] == 1) && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			double b = chance*100;
			int maxChance = (int) b;
			//System.out.println("disease: "+diseaseChance[index]+" "+hopsData.getDiseaseChance()+" "+maxChance);
			...
			if (Misc.random(100) <= maxChance) {
				...
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != hopsData.getStartingState()){
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateHopsStates();
				if (farmingStages[hopsFieldsData.getHopsIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[hopsFieldsData.getHopsIndex()] == 0) {
			...
			if(diseaseChance[hopsFieldsData.getHopsIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a hops patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[hopsFieldsData.getHopsIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a hops patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if(inspectData.getMessages().length > farmingStages[hopsFieldsData.getHopsIndex()] - 4)
						player.getDialogue().sendStatement(inspectData.getMessages()[farmingStages[hopsFieldsData.getHopsIndex()] - 4]);
					else
						player.getDialogue().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 1]);
					container.stop();
