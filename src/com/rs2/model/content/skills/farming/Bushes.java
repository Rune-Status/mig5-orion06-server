...
public class Bushes {
	...
	public enum BushesData {
		...
		REDBERRY(5101, 1951, 1, 10, new int[]{5478, 4}, 100, 0.20, 11.5, 4.5, 5, 14, 9, 58, 64), CADAVABERRY(5102, 753, 1, 22, new int[]{5968, 3}, 120, 0.20, 18, 7, 15, 25, 20, 59, 102.5), DWELLBERRY(5103, 2126, 1, 36, new int[]{5406, 3}, 140, 0.20, 31.5, 12, 26, 37, 32, 60, 177.5), JANGERBERRY(5104, 247, 1, 48, new int[]{5982, 6}, 160, 0.20, 50.5, 19, 38, 50, 45, 61, 284.5), WHITEBERRY(5105, 239, 1, 59, new int[]{6004, 8}, 160, 0.20, 78, 29, 51, 63, 58, 62, 437.5), POISONIVYBERRY(5106, 6018, 1, 70, null, 160, 0.20, 120, 45, 197, 209, 204, 63, 674);
	...
	public void doStateCalculation(int index) {
		...
		if (farmingState[index] == 0 && farmingStages[index] >= 5 && !hasFullyGrown[index]) {
			...
			if (Misc.random(100) <= maxChance) {
				...
				//if(Constants.DISEASING_ENABLED && farmingStages[index] != bushesData.getStartingState()){
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				updateBushesStates();
				if (farmingStages[bushesFieldsData.getBushesIndex()] == 3 && !alreadyStopped) {
					container.stop();
					return;
				}
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[bushesFieldsData.getBushesIndex()] == 0) {
			...
			if(diseaseChance[bushesFieldsData.getBushesIndex()] == SUPERCOMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a bush patch. The soil has been treated with supercompost.", "The patch is empty and weeded.");
				return true;
			}
			if(diseaseChance[bushesFieldsData.getBushesIndex()] == COMPOST_CHANCE){
				player.getDialogue().sendStatement("This is a bush patch. The soil has been treated with compost.", "The patch is empty and weeded.");
				return true;
			}
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if(inspectData.getMessages().length > farmingStages[bushesFieldsData.getBushesIndex()] - 4)
						player.getDialogue().sendStatement(inspectData.getMessages()[farmingStages[bushesFieldsData.getBushesIndex()] - 4]);
					else
						player.getDialogue().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 1]);
					container.stop();
