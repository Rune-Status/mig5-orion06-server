...
public class ArrowMaking {
	...
	public enum ArrowData {// you attach item 1 with item2
		...
		PEARL_BOLT(46, 877, 880, 41, 3.2), OPAL_BOLT(45, 877, 879, 11, 1.6);
	...
	public static boolean perform(final Player player, int itemUsed, int usedWith) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				String name = new Item(arrowData.getItem2()).getDefinition().getName().toLowerCase();
				player.getActionSender().sendMessage("You attach the " + new Item(arrowData.getItem1()).getDefinition().getName().toLowerCase() + " to " + count / multiplier + " " + name + (name.endsWith("s") ? "." : "s."));
