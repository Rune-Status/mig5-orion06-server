...
public class GemCrafting extends GemData {
	...
	public static void openInterface(Player player, int item) {
		if (!Constants.CRAFTING_ENABLED) {
			player.getActionSender().sendMessage("This skill is currently disabled.");
			return;
		}
	...
	public static void startCrafter(final Player player, final int gem, final int actionAmount, final int actionType) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (getData(gemType) == null || !player.getInventory().getItemContainer().contains(getData(gemType).mapLocation[0]) || !player.getInventory().getItemContainer().contains(player.getTempInteger())) {
					container.stop();
					return;
				}
				container.setTick(4);
				...
				switch (type) {
					...
					case 0 :
						...
						if (getData(gemType).mapLocation[0] != player.getTempInteger())
							player.getInventory().removeItem(new Item(getData(gemType).mapLocation[0], 1));
						player.getInventory().removeItem(new Item(player.getTempInteger(), 1));
						int item2Add = getData(gemType).mapLocation[5];
						if(player.getTempInteger() == 2365 && item2Add == 1641)
							item2Add = 773;
						player.getInventory().addItem(new Item(item2Add, 1));
					...
					case 1 :
						...
						if (getData(gemType).mapLocation[0] != player.getTempInteger())
							player.getInventory().removeItem(new Item(getData(gemType).mapLocation[0], 1));
						player.getInventory().removeItem(new Item(player.getTempInteger(), 1));
						item2Add = getData(gemType).mapLocation[8];
						if(player.getTempInteger() == 2365 && item2Add == 1660)
							item2Add = 774;
						player.getInventory().addItem(new Item(item2Add, 1));
	...
	public static void string(final Player player, final int gemSlot) {
		...
		player.getInventory().removeItem(new Item(1759, 1));
		player.getSkill().addExp(Skill.CRAFTING, 4);
