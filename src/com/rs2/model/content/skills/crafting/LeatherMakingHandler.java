...
import com.rs2.model.content.skills.crafting.LeatherMakingAction.impl.Splitbark;
...
public class LeatherMakingHandler {
	...
	public static boolean handleButtons(Player player, int buttonId, int amount) {
		...
		if (player.getStatedInterface() == "splitbark") {
			if (Splitbark.create(player, buttonId, amount) != null) {
				Splitbark.create(player, buttonId, amount).makeSplitbarkAction();
				return true;
			}
		}
		return false;
