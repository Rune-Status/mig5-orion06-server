...
public class AnagramsScrolls {
	...
	public static enum AnagramsData {
		...
		JARAAH(2678, 962, "Aha Jar", "", 3),
		CAROLINE(2679, 696, "Arc O Line", "Challenge", 2),
		...
		KARIM(2682, 543, "R Ak Mi", "Challenge", 3),
		...
		KING_ROALD(2693, 648, "Lark In Dog", "Challenge", 3),
	...
	public static boolean handleNpc(Player player, int npcId) {
		...
		if (!player.getInventory().getItemContainer().contains(anagramsData.getClueId())) {
			return false;
		}
		...
		if (anagramsData.getToGet() == "Challenge") {
			if (ChallengeScrolls.gotScroll(player, anagramsData.getClueId())) {
				...
				} else if (ChallengeScrolls.getString(anagramsData.getClueId()).length == 2){
				...
				} else if (ChallengeScrolls.getString(anagramsData.getClueId()).length == 3){
					player.getDialogue().sendNpcChat(ChallengeScrolls.getString(anagramsData.getClueId())[0], ChallengeScrolls.getString(anagramsData.getClueId())[1], ChallengeScrolls.getString(anagramsData.getClueId())[2], Dialogues.HAPPY);
				}
			} else {
				//player.getInventory().removeItem(new Item(anagramsData.getClueId(), 1));
				player.tempItem = new Item[1];
				player.tempItem[0] = new Item(anagramsData.getClueId(), 1);
			...
			if (Puzzle.finishedPuzzle(player) && player.hasPuzzle()) {
				Dialogues.setNextDialogue(player, 10009, 2);
				player.tempItem = new Item[4];
				player.tempItem[0] = new Item(anagramsData.getClueId(), 1);
				player.tempItem[1] = new Item(ClueScroll.CASTLE_PUZZLE, 1);
				player.tempItem[2] = new Item(ClueScroll.OGRE_PUZZLE, 1);
				player.tempItem[3] = new Item(ClueScroll.TREE_PUZZLE, 1);
				player.clueLevel = anagramsData.getLevel();
