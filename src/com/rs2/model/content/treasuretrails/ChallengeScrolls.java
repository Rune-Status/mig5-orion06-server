...
public class ChallengeScrolls {
	...
	public enum ChallengeData {
		...
		QUESTION_0(new String[]{"I have 16 kebabs, I eat one myself and", "share the rest equally between 3 friends.", "How many do they have each?"}, 2682, 2850, 5),
		QUESTION_4(new String[]{"How many cannons does Lumbridge", "castle have?"}, 2698, 2852, 9),
		QUESTION_5(new String[]{"How many animals are in", "the Ardougne Zoo?"}, 2686, 2854, 40),
		...
		QUESTION_15(new String[]{"How many bookcases are there ", "in the palace library?"}, 2693, 7269, 18);
	...
	public static void handleAnswer(Player player, int answer, int clueId) {
		...
		if (answer == challengeData.getAnswer()) {
			...
			player.tempItem = new Item[2];
			player.tempItem[0] = new Item(challengeData.getClueId(), 1);
			player.tempItem[1] = new Item(challengeData.getChallengeId(), 1);
		} else {
