...
public class TalkToEvent {
	...
	public static void spawnNpc(Player player, TalkToNpc n) {
		...
		player.getActionSender().sendSound(EventsConstants.RANDOM_EVENT_SOUND, 1, 0);
	...
	public static boolean isGenieLampButton(Player player, int actionButtonId) {
		...
		case 2831:
			...
			if (player.getGenieSelect() == -1) {
				...
				int iXp = player.getSkill().getAddedXp(player.getGenieSelect(), xp);
				player.getSkill().addExp(player.getGenieSelect(), xp);
				player.getActionSender().sendMessage("Congratulations, " + iXp + " xp was added to your " + Skill.SKILL_NAME[player.getGenieSelect()] + " skill.");
				player.getActionSender().sendSound(430, 1, 0);
