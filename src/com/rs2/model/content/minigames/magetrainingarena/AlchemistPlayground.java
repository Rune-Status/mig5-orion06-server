...
public class AlchemistPlayground {// todo guardian yelling
	...
	public int alchemistPizzazPoint;
	...
	private static int[] items = {6893, 6894, 6895, 6896, 6897};
	...
	private static void launchRotateEvent() { // todo make this an event instead
		...
		World.submit(new Tick(70) { // 42 seconds
			...
			public void execute() {
				rotateObjects();
				updatePrices();
				int temp = items[random.nextInt(items.length)];
				while(temp == freeCast){
					temp = items[random.nextInt(items.length)];
				}
				freeCast = temp;
				for (Player player : World.getPlayers()) {
					...
					if (player.getAlchemistPlayground().isInAlchemistPlayGround()){
						player.getAlchemistPlayground().showInterfaceComponent();
					}
				}
			}
		});
	...
	private static void removeItems(Player player) {
		player.getInventory().removeItem(new Item(boots, player.getInventory().getItemAmount(boots)));
		player.getInventory().removeItem(new Item(adamantKite, player.getInventory().getItemAmount(adamantKite)));
		player.getInventory().removeItem(new Item(adamantMedHelm, player.getInventory().getItemAmount(adamantMedHelm)));
		player.getInventory().removeItem(new Item(runeLongSword, player.getInventory().getItemAmount(runeLongSword)));
	...
	public void loadInterfacePlayer() {
		...
		player.getActionSender().sendString("" + alchemistPizzazPoint, 15896);// pizzaz
	...
	public void showInterfaceComponent() {
		removeInterfaceComponent();
		if (freeCast == boots)
			player.getActionSender().sendFrame171(0, 15907);
		else if (freeCast == adamantKite)
			player.getActionSender().sendFrame171(0, 15908);
		else if (freeCast == adamantMedHelm)
			player.getActionSender().sendFrame171(0, 15909);
		else if (freeCast == emerald)
			player.getActionSender().sendFrame171(0, 15910);
		else if (freeCast == runeLongSword)
			player.getActionSender().sendFrame171(0, 15911);
	}

	public void removeInterfaceComponent() {
		player.getActionSender().sendFrame171(1, 15907);
		player.getActionSender().sendFrame171(1, 15908);
		player.getActionSender().sendFrame171(1, 15909);
		player.getActionSender().sendFrame171(1, 15910);
		player.getActionSender().sendFrame171(1, 15911);
	}
	...
	public boolean handleObjectClicking(int objectId, int objectX, int objectY) { // todo
		...
		if (objectId == bootsObject || objectId == adamantKiteObject || objectId == adamantMedHelmObject || objectId == emeraldObject || objectId == runeLongSwordObject) {
			...
			if (isInAlchemistPlayGround())
				reward();
			return true;
		} else if (objectId == 10782 && isInAlchemistPlayGround()) {// leaving alchemist playground
			exit();
			return true;
	...
	public boolean isAlchemyItem(int itemId) {
	...
	private void reward() {
		...
		if (alchemistPizzazPoint + reward > MageGameConstants.MAX_ALCHEMY_POINT)
			reward = MageGameConstants.MAX_ALCHEMY_POINT - alchemistPizzazPoint;
		if(player.isCarryingProgressHat())
			alchemistPizzazPoint += reward;
