...
public enum RangedAmmo {
	...
	/* Arrows */
	BRONZE_ARROW(10, 19, 7), IRON_ARROW(9, 18, 10), STEEL_ARROW(11, 20, 16), MITHRIL_ARROW(12, 21, 22), ADAMANT_ARROW(13, 22, 31), RUNE_ARROW(15, 24, 49), CRYSTAL_BOW_ARROW(249, 250, 49),
			...
			if (ammoName.contains(ammo.name().toLowerCase())) {
				if (equipmentSlot != Constants.WEAPON) {
					...
					if (ammoName.contains("ogre")) {
						...
						int weaponRequiredLevel = new Item(weaponItem.getId()).getDefinition().getSkillReq(Skill.RANGED);
						int ammoRequiredLevel = new Item(ammoItem.getId()).getDefinition().getSkillReq(Skill.RANGED);
