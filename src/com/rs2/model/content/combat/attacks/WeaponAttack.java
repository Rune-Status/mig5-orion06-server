...
public class WeaponAttack extends BasicAttack {
    ...
    public AttackType getAtkType() {
		return getAttackStyle().getAttackType();
	}
	...
	public final void initialize() {
		...
		Item item = player.getEquipment().getItemContainer().get(Constants.WEAPON);
		String name = "";
		if (item != null){
			ItemDefinition def = ItemDefinition.forId(item.getId());
			name = def.getName().toLowerCase();
		}
		if(getAttackStyle().getAttackType() == AttackType.RANGED && name.contains("crystal bow")){
			rangedAmmo = RangedAmmo.CRYSTAL_BOW_ARROW;
		}
		else if (getAttackStyle().getAttackType() == AttackType.RANGED && !name.contains("crystal bow")) {
		...
		setSound(weapon.getWeaponInterface().atkSound);
