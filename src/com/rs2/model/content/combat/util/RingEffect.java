...
public class RingEffect {
	...
	public static void ringOfRecoil(Entity attacker, Player player, int damage) {
        ...
        if (ring != null && ring.getId() == 2550 && player != null && damage > 0) {
			...
			if (attacker != null) {
				HitRecord hitRecord = attacker.getHitRecord(player);
				if (hitRecord == null)
					hitRecord = new HitRecord(player);
				else
					attacker.getHitRecordQueue().remove(hitRecord);
				hitRecord.addDamage(damage);
				attacker.getHitRecordQueue().add(hitRecord);
			}
