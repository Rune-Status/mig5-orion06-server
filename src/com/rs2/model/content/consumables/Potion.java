	...
	public void drinkPotion(int itemId, int slot) {
		...
		if (player.getSkill().canDoAction2(600) && !player.isDead()) {
			...
			case 7919 : // Bottle of wine
				player.heal(11);
				player.getActionSender().sendMessage("You drink the " + potionDefinitions[potionIndex].getPotionName() + ".");
				player.getActionSender().statEdit(Skill.ATTACK, -5, false);
			...
			case 3801 : // Keg of beer
				...
				player.getActionSender().sendMessage("You drink the " + potionDefinitions[potionIndex].getPotionName() + "... that wasn't very smart!");
				player.heal(1);
				if (!player.getInventory().removeItemSlot(new Item(itemId, 1), slot)) {
					player.getInventory().removeItem(new Item(itemId, 1));
				}
			...
			case 1993 : // Jug of wine
				...
				player.getActionSender().sendMessage("You drink the " + potionDefinitions[potionIndex].getPotionName() + ".");
				if (player.getInventory().removeItemSlot(new Item(itemId, 1), slot)) {
					player.getInventory().addItemToSlot(new Item(1935), slot);
				} else if (player.getInventory().removeItem(new Item(itemId, 1))) {
					player.getInventory().addItem(new Item(1935));
				}
				return;
			case 1978 : // Cup of tea
				...
				player.heal(3);
				if (player.getInventory().removeItemSlot(new Item(itemId, 1), slot)) {
					player.getInventory().addItemToSlot(new Item(1980), slot);
				} else if (player.getInventory().removeItem(new Item(itemId, 1))) {
					player.getInventory().addItem(new Item(1980));
				}
				return;
			case 4627 : // bandits brew
				player.getActionSender().statEdit(Skill.DEFENCE, -6, false);
				player.getActionSender().statEdit(Skill.STRENGTH, -1, false);
				player.getActionSender().sendMessage("You drink the " + potionDefinitions[potionIndex].getPotionName() + "... it tashtes delishush!");
				player.getUpdateFlags().sendForceMessage("... hic!");
				if (player.getInventory().removeItemSlot(new Item(itemId, 1), slot)) {
					player.getInventory().addItemToSlot(new Item(1919), slot);
				} else if (player.getInventory().removeItem(new Item(itemId, 1))) {
					player.getInventory().addItem(new Item(1919));
				}
				return;	
			case 1913 : // Dwarven Stout
				player.getActionSender().statEdit(Skill.ATTACK, -5, false);
				player.getActionSender().statEdit(Skill.DEFENCE, -5, false);
				player.getActionSender().statEdit(Skill.STRENGTH, -5, false);
				player.getActionSender().sendMessage("You drink the " + potionDefinitions[potionIndex].getPotionName() + "... it tashtes delishush!");
				player.getUpdateFlags().sendForceMessage("... hic!");
				if (player.getInventory().removeItemSlot(new Item(itemId, 1), slot)) {
					player.getInventory().addItemToSlot(new Item(1919), slot);
			...
			if (potionIdIndex < 3) {
				...
				player.getActionSender().sendMessage("You drink the last of your " + potionDefinitions[potionIndex].getPotionName() + ".");
			}
		}
	}

	private boolean isDose(int itemId) {
		return itemId != 1993 && itemId != 1978 && itemId != 1917 && itemId != 1907 && itemId != 1913 && itemId != 4627 && itemId != 7919 && itemId != 3801;
	}
