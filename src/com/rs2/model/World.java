...
public class World {
	...
	public final static Player[] players = new Player[Constants.MAX_PLAYERS_AMOUNT];
	 ...
	 * @return the amount of online players
	 */
	public static int adminAmount() {
		int amount = 0;
		for (Player p : World.getPlayers()) {
			if (p != null) {
				if(p.getStaffRights() >= 2)
					amount++;
			}
		}
		return amount;
	...
	public static void yell(Player sender, String message) {
		for (Player player : players) {
			if (player == null) {
				continue;
			}
			player.getActionSender().sendMessage(sender, message);
		}
	}
