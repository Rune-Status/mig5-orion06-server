...
public class ClippedPathFinder {
	...
	public boolean findRoute(Player c, int destX, int destY, boolean moveNear, int xLength, int yLength) {
		if (destX == c.getPosition().getLocalX_2() && destY == c.getPosition().getLocalY_2() && !moveNear) {
			c.getActionSender().sendMessage("ERROR!");
			return false;
		}
		/*if (destX == c.getPosition().getLocalX(c) && destY == c.getPosition().getLocalY(c) && !moveNear) {
			c.getActionSender().sendMessage("ERROR!");
			return;
		}*/
		destX = destX - 8 * c.getPosition().getRegionX();
		destY = destY - 8 * c.getPosition().getRegionY();
		/*destX = destX - 8 * c.getCurrentRegion().getRegionX();
		destY = destY - 8 * c.getCurrentRegion().getRegionY()*/
		...
		int curX = c.getPosition().getLocalX_2();
		int curY = c.getPosition().getLocalY_2();
		/*int curX = c.getPosition().getLocalX(c);
		int curY = c.getPosition().getLocalY(c);*/
		via[curX][curY] = 99;
		cost[curX][curY] = 0;
		int tail = 0;
		tileQueueX.add(curX);
		...
		int l5;
		for (int j5 = l5 = via[curX][curY]; curX != c.getPosition().getLocalX_2() || curY != c.getPosition().getLocalY_2(); j5 = via[curX][curY]) {
			if (j5 != l5) {
				l5 = j5;
				tileQueueX.set(tail, curX);
				tileQueueY.set(tail++, curY);
		...
		/*for (int j5 = l5 = via[curX][curY]; curX != c.getPosition().getLocalX(c) || curY != c.getPosition().getLocalY(c); j5 = via[curX][curY]) {
			if (j5 != l5) {
				l5 = j5;
				tileQueueX.set(tail, curX);
				tileQueueY.set(tail++, curY);
