...
public class Region {
	...
	public static void addObject(int objectId, int x, int y, int height, int direction, int type, boolean startUp) {
		CacheObject obj = new CacheObject(GameObjectData.forId(objectId), new Position(x, y, height), type, direction);
	...
	public static int getClipping(int x, int y, int height) {
		...
		return 0x200000;//was 0 = no clip, now clipped
	}
   ...
   public static int[] getNextStep(int baseX, int baseY, int toX, int toY, int height, int xLength, int yLength) {
	...
	public static void load() {
		try {
			...
			byte[] buffer = new Archive(cache.getFile(0, 5)).getFile("map_index");
			Stream stream2 = new Stream(buffer);
			int size = buffer.length / 7;
			...
			for(int i2 = 0; i2 < size; i2++)
			{
				...
				mapGroundFileIds[i2] = stream2.readUnsignedWord();
				mapObjectsFileIds[i2] = stream2.readUnsignedWord();
			...
			for (int i = 0; i < size; i++) {
				byte[] file1 = ZipUtils.unzip1(cache.getFile(4, mapObjectsFileIds[i]));
				byte[] file2 = ZipUtils.unzip1(cache.getFile(4, mapGroundFileIds[i]));
