...
import com.rs2.net.packet.packets.ChatSettingsPacketHandler;
import com.rs2.net.packet.packets.CheatDetectPacketHandler;
...
import com.rs2.net.packet.packets.GePacketHandler;
...
import com.rs2.net.packet.packets.ReportPacketHandler;
...
public class PacketManager {
	...
	private static GePacketHandler ge = new GePacketHandler();
	...
	public static void loadPackets() {
		...
		packets[CheatDetectPacketHandler.CHEAT_DETECTION] = new CheatDetectPacketHandler();
		...
		packets[ReportPacketHandler.REPORT] = new ReportPacketHandler();
		packets[ChatSettingsPacketHandler.CHAT_SETTING] = new ChatSettingsPacketHandler();
		packets[GePacketHandler.SEARCH_ITEM] = ge;
		packets[0] = silent;
		packets[241] = silent;
		packets[86] = silent;
		packets[3] = silent;
	...
	public static void handlePacket(Player player, Packet packet) {
		...
		if (packet.getOpcode() == 243) {
			player.currentHiscorePage = 0;
			player.currentHiscoreSkill = 21;//inb4 crash but try it
			player.setHiscoreTableInfo();//try again
		}
		...
		try {
			if(!(packetHandler instanceof DefaultPacketHandler) && packet.getOpcode() != 202) {
				player.setIdleTimer(0);
			}
			player.lastPacketReceived = System.currentTimeMillis();
